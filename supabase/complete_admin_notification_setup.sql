-- Complete Admin and Notification System Setup
...
-- Step 6: Create RLS policies for admin table
...
-- PART 2: UPDATE NOTIFICATION SYSTEM
-- ==================================

-- Update the send_broadcast_notification function to use admin table
...
-- Update RLS policies for broadcast messages
...
GRANT EXECUTE ON FUNCTION send_broadcast_notification(text, text, text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION is_admin(text) TO authenticated;

-- PART 3: HELPFUL QUERIES
-- ======================

-- To add more admins, run:
-- INSERT INTO admin (email, created_by) VALUES ('new-admin@example.com', 'your-email@example.com');

-- To view all admins:
-- SELECT * FROM admin ORDER BY created_at;

-- To remove an admin:
-- DELETE FROM admin WHERE email = 'admin-to-remove@example.com';
